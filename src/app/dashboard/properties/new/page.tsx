...
export default function AddNewPropertyPage() {
  ...
  const [charges, setCharges] = useState<Charge[]>([{ id: "c1", title: "", amount: "" }]);
  ...
  return (
    <div className="flex flex-col" style={{ gap: "40px" }}>
          ...
          <div className="grid" style={{ gridTemplateColumns: "repeat(4, 1fr)", gap: "12px" }}>
            {photos.map((p, i) => (
              <div
                ...
                style={{ aspectRatio: "1", background: "#F6F6F6", borderRadius: "12px" }}
              >
                <Image src={URL.createObjectURL(p)} alt="" fill style={{ objectFit: "cover" }} unoptimized />
                <button
                  type="button"
      ...
      {/* (Action bar lives in the page header row — see top of page) */}
    </div>
  );
}
