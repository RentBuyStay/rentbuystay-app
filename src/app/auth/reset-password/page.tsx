...
/**
 * Password-reset landing — the reset email links here:
 * /auth/reset-password?email=…&code=…
 * Reads the emailed code, takes a new password, and confirms the reset in one
 * call. (The /forgot-password page is the manual email→code→password variant.)
 */
const HINTS = [
...
function ResetPasswordInner() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      await confirmReset({ email, code, newPassword: password }).unwrap();
      setSubmitted(true);
    } catch (err) {
      setError(
      ...
      <OnboardingShell>
        <div className="flex flex-col" style={{ gap: "8px" }}>
          ...
          <p style={{ fontSize: "16px", lineHeight: "24px", fontWeight: 400, color: "#807E7E", textAlign: "left" }}>
            {linkValid
              ? <>Choose a new password for{email ? <> <span style={{ color: "#305E82", fontWeight: 500 }}>{email}</span></> : " your account"}.</>
